 ...
 * 
 * IMPORTANT: This assumes existing data is in PKT (Asia/Karachi) timezone
...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    ...
    console.log('🔄 Starting migration: Convert events timestamp columns to timestamptz...');
    ...
    try {
      ...
      const hasOriginalEndTime = results.some(r => r.column_name === 'originalEndTime');
      ...
      const timestampColumns = ['startedAt', 'completedAt', 'stoppedAt', 'disabledAt'];
      for (const col of timestampColumns) {
        const exists = results.some(r => r.column_name === col);
        if (exists) {
          const isTimestamp = results.find(r => r.column_name === col)?.udt_name === 'timestamp';
          if (isTimestamp) {
            ...
            await queryInterface.sequelize.query(`
              ...
              ALTER COLUMN "${col}"
              TYPE TIMESTAMPTZ
              USING "${col}" AT TIME ZONE 'Asia/Karachi';
            `);
      ...
      console.log('📝 All timestamp columns are now TIMESTAMPTZ (with timezone)');
